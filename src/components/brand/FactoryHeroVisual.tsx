    ...
    <div className={`panel-raised stage-glow relative aspect-[4/3] overflow-hidden ${className}`}>
      <svg viewBox="0 0 480 360" className="absolute inset-0 h-full w-full" role="img" aria-label="Virtual rubber factory: polymer to finished roll">
        ...
        {Array.from({ length: 11 }).map((_, i) => (
          <line key={`v${i}`} x1={i * 48} y1="0" x2={i * 48} y2="360" stroke="rgba(15,23,42,0.05)" />
        ))}
        {Array.from({ length: 8 }).map((_, i) => (
          <line key={`h${i}`} x1="0" y1={i * 48} x2="480" y2={i * 48} stroke="rgba(15,23,42,0.05)" />
        ))}

        <text x="40" y="34" fill="#64748b" fontFamily="monospace" fontSize="11" letterSpacing="3">RUBBERFORGE · VAYUAI</text>

        {/* Polymer chain motif: monomer beads along a chain, with crosslink ties */}
        <g>
          <path d="M44 80 Q 70 64 96 80 T 148 80 T 200 80 T 252 80 T 304 80 T 356 80 T 408 80 T 436 80" fill="none" stroke="#7c3aed" strokeWidth="1.6" />
        ...
        <g>
          {[126, 150].map((cx, i) => (
            <g key={`roll${i}`}>
              <circle cx={cx} cy="211" r="13" fill="#cbd5e1" stroke="#64748b" strokeWidth="1.6" />
              <line x1={cx} y1="208" x2={cx} y2="214" stroke="#334155" strokeWidth="1.6">
        ...
        <g>
          <circle cx="206" cy="194" r="9" fill="#e2e8f0" stroke="#475569" strokeWidth="1.6" />
          <circle cx="206" cy="216" r="9" fill="#e2e8f0" stroke="#475569" strokeWidth="1.6" />
        </g>
        ...
        <g transform="translate(352, 209)">
          <path d="M-14 -4 h7 v-4 h6 v8 h-6 v-4 h-2 v4 h-2 v-4 h-3 z M14 -4 h-7 v-4 h-6 v8 h6 v-4 h2 v4 h2 v-4 h3 z M-9 -1 h18 v2 h-18 z" fill="none" stroke="#0891b2" strokeWidth="1.6" />
          <path d="M-4 8 l3 3 l6 -7" fill="none" stroke="#0891b2" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
        </g>
        ...
        <g>
          ...
          <circle cx="416" cy="209" r="1.8" fill="#2563eb" />
        </g>

        {/* Traceability digital thread connecting every stage */}
        <line x1="60" y1="252" x2="420" y2="252" stroke="#cbd5e1" strokeWidth="2" />
        <line x1="60" y1="252" x2="288" y2="252" stroke="url(#fh-thread)" strokeWidth="2.4" />
        {[67, 138, 206, 288, 352, 416].map((x, i) => (
          <circle key={`tn${i}`} cx={x} cy="252" r="3.6" fill="#ffffff" stroke={i === 3 ? "#d97706" : "#0891b2"} strokeWidth="1.8" />
        ))}
        <circle cy="252" r="4" fill="#d97706">
          <animate attributeName="cx" values="67;416;67" dur="6s" repeatCount="indefinite" />
        </circle>

        <text x="40" y="300" fill="#475569" fontFamily="monospace" fontSize="9.5" letterSpacing="1.5">
          POLYMER → COMPOUND → PROCESS → CURE → QUALITY → OUTPUT
        </text>
        <text x="40" y="320" fill="#94a3b8" fontFamily="monospace" fontSize="8.5" letterSpacing="2">DIGITAL TWIN-STYLE VIRTUAL RUBBER FACTORY</text>
      </svg>
    </div>
  );
}
